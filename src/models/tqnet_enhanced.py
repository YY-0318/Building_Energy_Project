...
class TQAttention(nn.Module):
    def __init__(self, pred_len, d_model, n_heads=4, dropout=0.1, tau=1.0):
        super().__init__()
        self.tau = tau # 温度系数
    ...
    def forward(self, history_enc):
        ...
        query = (self.query_embedding / self.tau).unsqueeze(0).repeat(batch_size, 1, 1)
        attn_out, attn_weights = self.mha(query, history_enc, history_enc)
        x = self.norm1(query + attn_out)
        x = self.norm2(x + self.ffn(x))
        return x, attn_weights

class ShortTermTQNet(nn.Module):
    def __init__(self, input_dim, seq_len, pred_len, d_model=128, n_heads=4, kernel_size=3, tau=1.0,
                 use_revin=True, use_cnn=True, use_attention=True, use_trend=True): 
        super().__init__()
        ...
        if self.use_attention:
            self.tq_block = TQAttention(pred_len, d_model, n_heads=n_heads, tau=tau)
    ...
    def forward(self, x):
        ...
        # 3. TQAttention
        if self.use_attention:
            tq_out, weights = self.tq_block(x_enc)
